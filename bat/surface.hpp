...
public:
  ...
  Surface() {}
  Surface(const char*img);
};

inline int stringLen(const char*s1) {
	int i = 0;
	while (s1[i]) i++;
	return i;
}

inline int sameString(const char*s1, char*s2, int len) {
  for (int i = 0; i < len; i++) 
    if (s1[i] != s2[i]) return 0;
  return 1;
}
inline void fskip(FILE *fp, int n) {
  while (n--) fgetc(fp);
}

inline Surface::Surface(const char*img) {
  int i;
