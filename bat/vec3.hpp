...
public:
  ...
  void operator*=(float);
  void operator/=(float);
  unsigned char operator==(vec3 v) {
    return (v.x == x && v.y == y && v.z == z);
  }
  float&operator[](int);
  vec3 normalize(float);
  void rotate(vec3, vec3, vec3);
  void rotateR(vec3, vec3, vec3);
  void rotatePlane(vec3, float);
};
...
float pers = 1;
vec3*lights, camera(0, 0, 0), viewdir(0, 0, 1), viewing(0, 0, 0), up(0, 1, 0);
int numlights = 0;

void Trackball(float sx, float sy, float ex, float ey) {
  float f;
  if (sx*sx+sy*sy > 1) {
