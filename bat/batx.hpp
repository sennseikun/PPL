...
inline int code2key(Display*d, int kc) {
  int tmp;
...
public:
  ...
  XImage * pixmapImage;
  MyScreen() {}
  MyScreen(unsigned int, unsigned int);
  MyScreen(Display*, unsigned int, unsigned int);
  void update();
  void flush();
  void putSurface(Surface);
  void getCursorPos(int&x, int&y);
  void hideCursor();
  Event getEvent();
  int gotEvent();
  void setCursorPos(int, int);
  void init(Display*, unsigned int, unsigned int);
};

inline void MyScreen::hideCursor() {
  Cursor no_ptr;
  Pixmap bm_no;
  XColor black, dummy;
  Colormap colormap;
...
inline void MyScreen::init(Display*disp, unsigned int sw, unsigned int sh) {
  w = sw;
  h = sh;
  display = disp;
...
inline MyScreen::MyScreen(Display*disp, unsigned int sw, unsigned int sh) {
  init(disp, sw, sh);
}

inline MyScreen::MyScreen(unsigned int sw, unsigned int sh) {
  init(XOpenDisplay(NULL), sw, sh);
}

inline int MyScreen::gotEvent() {
...
inline Event MyScreen::getEvent() {
	XEvent e;
...
inline void MyScreen::flush() {
  XFlush(display);
}

inline void MyScreen::update() {
  XPutImage(display, win, gc, pixmapImage, 0, 0, 0, 0, w, h);
  XFlush(display);
}

inline void MyScreen::putSurface(Surface sf) {
  static int warned = 0;
...
inline void MyScreen::getCursorPos(int&x, int&y) {
  unsigned int r;
  int rx, ry;
  Window w;
...
inline void MyScreen::setCursorPos(int x, int y) {
